# -*- coding: utf-8 -*-
"""
Created on Thu Jun 26 11:21:12 2025

@author: says
"""

#SUBSTITUTION IN QUADRATIC OPTISATION
import streamlit as st
import pandas as pd
import altair as alt


def model_quad_substitute():
    ...
    options = st.sidebar.multiselect(
        "What food do you want to eliminate from your diet?",
        list(food_binary_map.keys()),
        default=["Milk"]
    )

    st.sidebar.write("You selected:", options)
    # Calculate binary sum based on selection
    binary_sum = sum([food_binary_map[food] for food in options])
    ...
    if binary_sum >23:
        ...
        df = pd.read_excel("./SUBSTITUTION.xlsx")
        daly = pd.read_excel("./SUBSTITUTION.xlsx", sheet_name='DALY', header = 0)
        slack = pd.read_excel("./SUBSTITUTION.xlsx", sheet_name='SLACK', header = 0)
        ...
        df_long["CF"] = df_long["CF"].str.extract(r'A(\d+)', expand=False).astype(int)
        data_filtered = df_long[(df_long['CF'] ==  binary_sum )]
        ...
        df_long1["CF"] = df_long1["CF"].str.extract(r'A(\d+)', expand=False).astype(int)
        data_filtered1 = df_long1[(df_long1['CF'] ==  binary_sum )]
        ...
        daly_long['CF'] = daly_long["CF"].str.extract(r'A(\d+)', expand=False).astype(int)
        ################### DATA PREPROCESSING FOR DALY CHART ###########################################
        daly_filtered = daly_long[(daly_long['CF'] ==  binary_sum )]
        ...
        slack_long['CF'] = slack_long['CF'].str.extract(r'A(\d+)', expand=False).astype(int)

        ############## DATA PREPROCESSING FOR SLACK CHART ##########################################
        slack_filtered = slack_long[(slack_long['CF'] == binary_sum  )]
